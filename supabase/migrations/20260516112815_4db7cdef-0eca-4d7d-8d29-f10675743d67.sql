...
BEGIN
  ...
  BEGIN
    ...
    PERFORM public.cancel_sale(_sale_id => v_sale, _reason => 'test cancellation'::text, _user_id => v_user);
    ...
    BEGIN PERFORM public.cancel_sale(_sale_id => v_sale, _reason => 'second cancel'::text, _user_id => v_user);
    ...
    PERFORM public.cancel_sale(_sale_id => v_sale, _reason => 'cancel crediario'::text, _user_id => v_user);
    SELECT COALESCE(qty,0) INTO v_qty_a_after_cancel FROM public.inventory
      WHERE account_id=v_account AND store_id=v_store AND product_id=v_prod_a AND variant_id IS NULL;
    ...
    SELECT COUNT(*) INTO v_recv_open FROM public.accounts_receivable
      WHERE sale_id = v_sale AND status NOT IN ('cancelled','paid');
