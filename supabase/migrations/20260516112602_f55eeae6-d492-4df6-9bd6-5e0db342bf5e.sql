...
BEGIN
  ...
  BEGIN
    ...
    PERFORM public.cancel_sale(sale_id => v_sale, reason => 'test cancellation'::text);
    ...
    BEGIN PERFORM public.cancel_sale(sale_id => v_sale, reason => 'second cancel'::text);
    ...
    PERFORM public.cancel_sale(sale_id => v_sale, reason => 'cancel crediario'::text);
    SELECT COALESCE(qty,0) INTO v_qty_a_after_cancel FROM public.inventory
      WHERE account_id=v_account AND store_id=v_store AND product_id=v_prod_a AND variant_id IS NULL;
