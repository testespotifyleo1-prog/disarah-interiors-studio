...
BEGIN
  ...
  IF NEW.status = 'paid' AND OLD.status != 'paid' THEN
    ...
    -- 3a. Check seller_commission_rules first (fixed percentage)
    ...
    -- 3b. If no fixed rule, check commission_tiers
    IF v_commission_percent = 0 THEN
      -- Check per_sale tiers first
      ...
      WHERE account_id = NEW.account_id
        ...
        AND is_active = true
        AND NEW.total >= min_value
        AND (max_value IS NULL OR NEW.total <= max_value)
      ORDER BY min_value DESC
      LIMIT 1;
      
      -- If no per_sale tier, check monthly_accumulated
      IF v_commission_percent IS NULL OR v_commission_percent = 0 THEN
        -- Calculate monthly total including this sale
