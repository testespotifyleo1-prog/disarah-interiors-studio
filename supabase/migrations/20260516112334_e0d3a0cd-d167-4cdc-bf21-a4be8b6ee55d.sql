...
BEGIN
  ...
  BEGIN
    ...
    VALUES (v_account, '__TEST_PROD_A__', '__TST_A__'||gen_random_uuid()::text, 100, 50, true)
    RETURNING id INTO v_prod_a;

    INSERT INTO public.products(account_id, name, sku, price_default, cost_default, is_active)
    VALUES (v_account, '__TEST_PROD_B__', '__TST_B__'||gen_random_uuid()::text, 200, 100, true)
    RETURNING id INTO v_prod_b;

    INSERT INTO public.product_variants(account_id, product_id, sku, attributes, is_active)
    VALUES (v_account, v_prod_b, '__TST_B_M__'||gen_random_uuid()::text, '{"size":"M"}'::jsonb, true)
    ...
    VALUES (v_account, v_store, 'paid', 500, 500, 0, 0, 0, 0, 'pos', false, 0, 0)
    ...
    RETURN QUERY SELECT 'stock_deduct_null_variant'::text,
      (v_qty_a_after_sale = v_qty_a_before - 3),
      format('before=%s after=%s', v_qty_a_before, v_qty_a_after_sale);
    RETURN QUERY SELECT 'stock_deduct_with_variant'::text,
    ...
    RETURN QUERY SELECT 'stock_restore_null_variant_after_cancel'::text,
      (v_qty_a_after_cancel = v_qty_a_before),
      format('expected=%s got=%s', v_qty_a_before, v_qty_a_after_cancel);
    RETURN QUERY SELECT 'stock_restore_with_variant_after_cancel'::text,
    ...
    RETURN QUERY SELECT 'movements_created_for_sale'::text,
      (v_mov_count >= 4), format('movements=%s', v_mov_count);

    BEGIN PERFORM public.cancel_sale(v_sale, 'second cancel');
    EXCEPTION WHEN OTHERS THEN NULL; END;

    SELECT COALESCE(qty,0) INTO v_qty_a_after_cancel FROM public.inventory
      WHERE account_id=v_account AND store_id=v_store AND product_id=v_prod_a AND variant_id IS NULL;
    RETURN QUERY SELECT 'cancel_idempotent_null_variant'::text,
      (v_qty_a_after_cancel = v_qty_a_before),
      format('got=%s', v_qty_a_after_cancel);

    INSERT INTO public.sales(account_id, store_id, status, total, subtotal, discount, freight, delivery_fee, assembly_fee, source, payment_on_delivery, down_payment, remaining_balance)
    VALUES (v_account, v_store, 'crediario', 100, 100, 0, 0, 0, 0, 'pos', false, 0, 100)
    RETURNING id INTO v_sale;
    ...
    SELECT COALESCE(qty,0) INTO v_qty_a_after_sale FROM public.inventory
      WHERE account_id=v_account AND store_id=v_store AND product_id=v_prod_a AND variant_id IS NULL;
    RETURN QUERY SELECT 'crediario_deducts_stock'::text,
    ...
    SELECT COALESCE(qty,0) INTO v_qty_a_after_cancel FROM public.inventory
      WHERE account_id=v_account AND store_id=v_store AND product_id=v_prod_a AND variant_id IS NULL;
    RETURN QUERY SELECT 'crediario_restores_stock_on_cancel'::text,
      (v_qty_a_after_cancel = v_qty_a_before),
      format('expected=%s got=%s', v_qty_a_before, v_qty_a_after_cancel);
    ...
    RETURN QUERY SELECT 'receivables_cancelled_with_sale'::text,
      (v_recv_open = 0), format('open=%s', v_recv_open);
