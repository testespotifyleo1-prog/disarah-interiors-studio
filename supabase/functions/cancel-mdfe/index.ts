...
serve(async (req) => {
  ...
  try {
    ...
    if (!mdfe_id || !justificativa) throw new Error('mdfe_id e justificativa obrigatórios');
    if (justificativa.length < 15) throw new Error('Justificativa precisa de no mínimo 15 caracteres');
    ...
    const resp = await fetch(`${baseUrl}/v2/mdfe/${doc.provider_ref}`, {
      method: 'DELETE',
